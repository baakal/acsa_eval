...
export default function Home() {
  ...
  return <main className="assessmentApp">
    <section className="content requirementsContent">
      <header>
        <div className="assessmentBrand"><span className="mark">A</span><span>ACSA <b>Evaluation</b></span><i></i><div><strong></strong><small>{answered} of {catalogue.length} complete</small></div><div className="scoreSummary"><strong>{complianceScore}%</strong><small>Compliance score</small></div></div>
      ...
      {workspaceView === 'home' ? <div className="homePage">
        ...
        <section className="homeSectionTitle"><div><span className="eyebrow">REQUIREMENTS CATALOGUE</span><h2>Questionnaire at a glance</h2><p>Fixed statistics describing the ACSA requirements catalogue itself, independent of your progress.</p></div></section>
        <section className="homeStats"><article><span>Total requirements</span><strong>{catalogue.length}</strong><small>{functionalCount} functional · {nonFunctionalCount} non-functional</small></article><article><span>Requirement categories</span><strong>{categories.length}</strong><small>Assessment areas in the catalogue</small></article><article><span>Must-have requirements</span><strong>{mustCount}</strong><small>{shouldCount} should · {couldCount} could</small></article><article><span>Maximum achievable score</span><strong>{maxWeightedScore.toFixed(1)}</strong><small>Weighted by priority — Must ×2, Should ×1.5, Could ×1</small></article></section>
